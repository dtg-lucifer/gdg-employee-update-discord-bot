...
class Tools (Cog):
    ...
    @commands.command(name="booster", description="Shows a list of boosters", aliases=["boost", "serverboosters", "boosters"])
    async def booster(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            title=f"✨ Server Boosters for {ctx.guild.name}",
        ...
        for booster in boosters:
            join_date = booster.premium_since.strftime("%b %d, %Y") if booster.premium_since else "Unknown"
            embed.add_field(
                name=f"👤 {booster.display_name}",
                value=f"🆔 `{booster.id}`\n🚀 Boosting since: **{join_date}**",
                inline=False
            )
